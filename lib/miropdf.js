...
*/
		...
		if (outline.y + outline.height > this.parentBB.y + this.parentBB.height) {
			cfg.reqNewPage();
	...
	function renderSVG(svg) {
		...
		try {
			svgo.optimize(svg, function(mind) {
				var itemw = cfg.parseWidth(item.width)
				if (itemw < 0) itemw *= -cfg.parentBB.width;
...
// 				self.bgAndBorder(dobbs.bg, style, cfg);
				...
				cfg.pdf.transform(width / mind.info.width, 0, 0, height / mind.info.height, 0, 0);
...
DrawManager.prototype.genTextQueue = function(text, startbb, xOffset, style, cfg, flushFn) {
	...
	for (var i = 0; i < paragraphs.length; ++i) {
		...
		if (!style.ignoreNewPage && curRect.y + curRect.height > cfg.parentBB.y + cfg.parentBB.height) {
...
DrawManager.prototype.drawText = function(item, cfg) {
	...
	this.genTextQueue(item.content, null, xOffset, style, cfg, function(queue, done, newPage) {
		self.drawTextQueue(queue, cfg, style, function(lastBBs) {
			cfg.parentBB.addBB(lastBBs);
			done && cfg.itemDrawn && cfg.itemDrawn();
		});
	});
}
...
DrawManager.prototype.drawTable = function(item, cfg) {
	...
	rows.forEach(function(row, i) {
		row.forEach(function(cell, j) {
			...
			style = styleManager.extendStyle(styleManager.extendStyle(style, colData[i]), { width: null, ignoreNewPage: true });
