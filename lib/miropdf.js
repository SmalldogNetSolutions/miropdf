...
var PDFWrapper = function() {
	...
	this.finish = function(done) {
		...
		ws.on("finish", function() {
			...
			log("Writing file " + outPath + "\n\tGenerated in " + (elapsedTime / 1000).toPrecision(3) + " seconds.", id);
			done();
		});
		doc.end();
	...
	this.render = function(json, done) {
		...
		docCount++;
		debug && log("Rendering doc " + docCount);
...
PDFArtist.prototype.drawPNG = function(item, cfg) {
	...
	function renderPNG() {
		if (!cachedImg) self.pushImgCache(item.content.path || item.content.url, img);
