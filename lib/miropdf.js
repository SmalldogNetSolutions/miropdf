			...
			try {
				...
				run(output_file, docs, function() {
					...
					if (stats.isFile()) {
						...
						console.log("Build success! '" + outerJSON.output_file + "'");
					} else {
						res.end(JSON.stringify({
							success: 0,
...
function run(outPath, pdfList, done) {
	...
	var wrapper = new PDFWrapper(StyleManager);
...
var PDFWrapper = function() {
	...
	this.finish = function(done) {
		// doc.moveTo(10, 10).addContent("Do /test");
  ...
  //      		ProcSet: [ "PDF" ]
  //      	});
  //      	test.data["BBox"] = [ 0, 0, 50, 50];
  //      	test.data["Name"] = [ "test" ];
  //      	test.data["Resources"] = resources;
  //      	resources.end();

		// var oldAdd = doc.addContent;
		// var str = "";
		// doc.addContent = function(content) {
		// 	str += content + "\n";
		// 	return doc;
		// }
  //      	doc.rect(10, 10, 30, 20).lineWidth(5).stroke("black");
  //      	doc.addContent = oldAdd;
  //      	test.end(str.trim());
	...
	this.render = function(json, done) {
		...
		pdfArtist = new PDFArtist(this, json);
...
function PDFArtist(pdfWrapper, json) { this.imgCache = {}; this.pdfWrapper = pdfWrapper; this.json = json; };
